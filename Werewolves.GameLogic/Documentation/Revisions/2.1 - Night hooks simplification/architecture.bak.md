...
16. **Enums:** 
    ...
    *   `PhaseTransitionReason` (Enum used internally by `GameFlowManager` and `PhaseHandlerResult`): 
        ...
        *   `NightRoleIdentificationComplete`: Transition within Night phase after first-night role identification 
    ...
    *   **Sub-Phase Enums:**
        *   `NightSubPhases`: `Start`, `RoleIdentification`, `ActionLoop` - Internal sub-phases for consolidated Night phase
...
2.  **Night Phase (`GamePhase.Night`):** 
    ...
    *   Handler retrieves current sub-phase from `GamePhaseStateCache` and routes to appropriate sub-handler: 
        *   **NightSubPhase.Start**: Issues "Village goes to sleep" instruction, increments turn number, transitions to `RoleIdentification`. 
        *   **NightSubPhase.RoleIdentification** (Turn 1 only): Processes first-night role identification (Thief, Cupid, etc.) using hook system, transitions to `ActionLoop`. 
        *   **NightSubPhase.ActionLoop**: Iterates through complete ordered list of night roles, fires appropriate `GameHook` for each, manages pause/resume via `GamePhaseStateCache`, transitions to `Day_Dawn` when complete. 
