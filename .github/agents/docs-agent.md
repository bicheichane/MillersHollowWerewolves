...
3. **Assess Intentional Divergences**: Check for any documented divergences in `Documentation/AgentFeedback/QA/implementation-divergences.md` and `Documentation/AgentFeedback/Coder/implementation-divergences.md`. Incorporate these divergences into your understanding of the intended changes. Note that it is possible for these files to be stale, and relate to previous changes; if it's not obvious if they relate to the current changes, ALWAYS ask the user for clarification via `ask_user`.
