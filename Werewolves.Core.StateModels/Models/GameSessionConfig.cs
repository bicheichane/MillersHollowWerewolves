...
namespace Werewolves.Core.StateModels.Models
{
	public class GameSessionConfig
	{
		...
		public static bool TryGetConfigIssues(List<string> players, List<MainRoleType> roles, out List<GameConfigValidationError> issues)
		{
			...
			var expectedPlayerRoleCountDiff = (roles.Contains(Thief) ? 2 : 0) + (roles.Contains(Actor) ? 3 : 0);
			...
			if (players.Count < 1)
			{
				issues.Add(new GameConfigValidationError(GameConfigValidationErrorType.TooFewPlayers, "At least one player is required."));
			}
			...
			foreach (var kv in RoleCountConstraints)
			{
				var role = kv.Key;
				var constraint = kv.Value;
				var count = roles.Count(r => r == role);

				if (constraint.IsValid(roles))
				{
